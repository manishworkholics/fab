...
const Project = () => {
  ...
  /* =====================================================
     Filter projects based on new tabs
  ===================================================== */
  ...
  ===================================================== */
  const counts = {
    newBids: projects.length,
    ongoing: projects.filter((p) =>
      [ProjectStatus.Assigned, ProjectStatus.InProgress].includes(p.status)
    ).length,
    favourites: 0,
  ...
  return (
    <DasboardLayout>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mt-8">
        <StatCard label="New Bids" value={counts.newBids} />
        <StatCard label="Ongoing Projects" value={counts.ongoing} />
        <StatCard label="Favourites" value={counts.favourites} />
        <StatCard label="Completed Deals" value={counts.completed} />
      </div>
            ...
            {!loading &&
              filteredProjects.map((p) => (
                <tr
                  key={p.id}
                  className="border-b hover:bg-gray-50 cursor-pointer"
                  onClick={() => navigate(`/ems/projects/${p.id}`)}
                >
                  <td className="p-3 font-medium">
                    {p.pm?.firstName} {p.pm?.lastName}
                  </td>
