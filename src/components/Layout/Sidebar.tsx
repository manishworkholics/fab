...
const Sidebar = ({ role, toggleSideBar, isSideBar }: SearchBarProps) => {
  ...
  // ✅ show sidebar only if current route exists in sidebar menu
        ...
        .map((data) => (
          <button
            ...
            className={`flex items-center px-4 py-4 text-sm font-medium rounded-md w-full ${
              location.pathname.startsWith(data.label)
                ? "text-[#EB5017] bg-orange-100"
                : "text-[#344054]"
            }`}
            onClick={() => {
              navigate(data.label);
  ...
  return (
    <>
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="lg:hidden fixed top-16 left-0 w-full bg-white shadow z-20 p-4">
          <SearchBar placeHolderText="search" background="bg-transparent" textIcon />
          {renderNavItems()}
        </div>
      )}
      ...
      {/* Desktop Sidebar */}
      {isSideBar && (
        <aside className="hidden lg:flex fixed left-0 top-0 w-64 bg-white shadow-md flex-col z-30 h-screen overflow-y-auto">
          ...
          <div className="p-4 border-t">
            <div className="flex items-center">
              <img src="/images/user-2.png" className="w-10 h-10 rounded-full" />
              <div className="ml-3">
